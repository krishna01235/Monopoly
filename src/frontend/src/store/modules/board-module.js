...
export default {
  ...
  actions: {
    async getNewBoard({ state, commit, dispatch }, { players }) {
      // This is a local-only game creation - no blockchain transactions are triggered
      console.log('Creating new board locally - no blockchain interactions')
    ...
    async throwDice({ state, commit, dispatch, getters }, { dice }) {
      // Local-only dice throw - no blockchain transactions
      console.log('Throwing dice locally - no blockchain interactions')
    ...
    async swichToNextPlayer({ state, commit, getters }) {
      // Local-only player switch - no blockchain transactions
      console.log('Switching to next player locally - no blockchain interactions')
      try {
        let copyBoard = JSON.parse(JSON.stringify(state.board))
